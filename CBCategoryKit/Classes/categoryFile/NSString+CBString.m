...
+ (void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method newMethod = class_getInstanceMethod(class, @selector(CB_substringFromIndex:));
        if (oldMethod && newMethod) {
            method_exchangeImplementations(oldMethod, newMethod);
        }
        
        Method oldM = class_getInstanceMethod(class, @selector(substringToIndex:));
        Method newM = class_getInstanceMethod(class, @selector(CB_substringToIndex:));
        if (oldM && newM) {
...
- (NSString *)CB_substringFromIndex:(NSUInteger)from{
    if (self.length<from) {
        NSLog(@"字符串substringFromIndex方法越界");
        return @"";
    }else{
        return [self CB_substringFromIndex:from];
    }
}
